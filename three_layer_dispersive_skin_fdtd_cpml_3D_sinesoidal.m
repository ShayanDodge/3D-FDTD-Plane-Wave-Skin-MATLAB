...
material_type(2).debye_tau=7.386.*10^-12;
...
material_type(3).debye_tau=6.20.*10^-12;
...
material_type(4).debye_tau=2.14.*10^-12;
...
cpmlxnvec=([1:1:n_cpml_xn]);
cpmlxpvec=([nx-n_cpml_xp:1:nx-1]);

cpmlynvec=([1:1:n_cpml_yn]);
cpmlypvec=([ny-n_cpml_yp:1:ny-1]);

cpmlznvec=([1:1:n_cpml_zn]);
cpmlzpvec=([nz-n_cpml_zp:1:nz-1]);
%% ====== Sine Source ===========
r=0.25.*10^-3;R=2.*10^-3;
voltage=6000;
...
% % % Coeffiecients updating Hx
Chxh((nx+1)/2,((ny+1)/2)-1,n_cpml_zn +2)=(1);
Chxez((nx+1)/2,((ny+1)/2)-1,n_cpml_zn+2)=-(2.*dt./(mu_0.*mu_r_x((nx+1)/2,((ny+1)/2)-1,n_cpml_zn+2).*dy.*log(dy./r)));
Chxey((nx+1)/2,((ny+1)/2)-1,n_cpml_zn+2)=(dt./(mu_0.*mu_r_x((nx+1)/2,((ny+1)/2)-1,n_cpml_zn+2).*dz));
Chxh((nx+1)/2,((ny+1)/2),n_cpml_zn +2)=(1);
Chxez((nx+1)/2,((ny+1)/2),n_cpml_zn+2)=-(2.*dt./(mu_0.*mu_r_x((nx+1)/2,((ny+1)/2),n_cpml_zn+2).*dy.*log(dy/r)));
Chxey((nx+1)/2,((ny+1)/2),n_cpml_zn+2)=(dt./(mu_0.*mu_r_x((nx+1)/2,((ny+1)/2),n_cpml_zn+2).*dz));
% % 
% % % % Coeffiecients updating Hy
Chyh(((nx+1)./2)-1,(ny+1)/2,n_cpml_zn   +2)=1;
Chyex(((nx+1)./2)-1,(ny+1)/2,n_cpml_zn  +2)=-dt./(mu_0.*mu_r_y(((nx+1)./2)-1,(ny+1)/2,n_cpml_zn+2).*dz);
Chyez(((nx+1)./2)-1,(ny+1)/2,n_cpml_zn +2)=2.*dt./(mu_0.*mu_r_y(((nx+1)/2)-1,(ny+1)/2,n_cpml_zn+2).*dx.*(log(dx./r)));
Chyh(((nx+1)./2),(ny+1)/2,n_cpml_zn +2)=1;
Chyex(((nx+1)./2),(ny+1)/2,n_cpml_zn+2)=-dt./(mu_0.*mu_r_y(((nx+1)./2),(ny+1)/2,n_cpml_zn+2).*dz);
Chyez(((nx+1)./2),(ny+1)/2,n_cpml_zn+2)=2.*dt./(mu_0.*mu_r_y(((nx+1)/2),(ny+1)/2,n_cpml_zn+2).*dx.*log(dx/r));
...
Psi_hyx_xp=cpml_b_mx_xp.*Psi_hyx_xp+cpml_a_mx_xp.*(Ez(cpmlxpvec+2,:,:)-Ez(cpmlxpvec+1,:,:));
Psi_hzx_xp=cpml_b_mx_xp.*Psi_hzx_xp+cpml_a_mx_xp.*(Ey (cpmlxpvec+2,:,:)-Ey(cpmlxpvec+1,:,:));
Hy(cpmlxpvec+1,:,:)=Hy(cpmlxpvec+1,:,:)+CPsi_hyx_xp.*Psi_hyx_xp;
Hz(cpmlxpvec+1,:,:)=Hz(cpmlxpvec+1,:,:)+CPsi_hzx_xp.*Psi_hzx_xp;

Psi_hxy_yn=cpml_b_my_yn.*Psi_hxy_yn+cpml_a_my_yn.*(Ez(:,cpmlynvec+1,:)-Ez(:,cpmlynvec,:));
Psi_hzy_yn=cpml_b_my_yn.*Psi_hzy_yn+cpml_a_my_yn.*(Ex(:,cpmlynvec+1,:)- Ex(:,cpmlynvec,:));
Hx(:,cpmlynvec,:)=Hx(:,cpmlynvec,:)+CPsi_hxy_yn.*Psi_hxy_yn;
Hz(:,cpmlynvec,:)=Hz(:,cpmlynvec,:)+CPsi_hzy_yn.*Psi_hzy_yn;

Psi_hxy_yp=cpml_b_my_yp.*Psi_hxy_yp+cpml_a_my_yp.*(Ez(:,cpmlypvec+2,:)-Ez(:,cpmlypvec+1,:));
Psi_hzy_yp=cpml_b_my_yp.*Psi_hzy_yp+cpml_a_my_yp.*(Ex(:,cpmlypvec+2,:)-Ex(:,1+cpmlypvec,:));
Hx(:,cpmlypvec+1,:)=Hx(:,cpmlypvec+1,:)+CPsi_hxy_yp.*Psi_hxy_yp;
Hz(:,cpmlypvec+1,:)=Hz(:,cpmlypvec+1,:)+CPsi_hzy_yp.*Psi_hzy_yp;

Psi_hxz_zn=cpml_b_mz_zn.*Psi_hxz_zn+cpml_a_mz_zn.*(Ey(:,:,cpmlznvec+1) - Ey(:,:,cpmlznvec));
Psi_hyz_zn=(cpml_b_mz_zn).*Psi_hyz_zn+(cpml_a_mz_zn).*(Ex(:,:,cpmlznvec+1) - Ex(:,:,cpmlznvec));
Hx(:,:,cpmlznvec)=Hx(:,:,cpmlznvec)+CPsi_hxz_zn.*Psi_hxz_zn;
Hy(:,:,cpmlznvec)=Hy(:,:,cpmlznvec)+CPsi_hyz_zn.*Psi_hyz_zn;

Psi_hyz_zp=(cpml_b_mz_zp).*Psi_hyz_zp+(cpml_a_mz_zp).*(Ex(:,:,cpmlzpvec+2)-Ex(:,:,cpmlzpvec+1));
Psi_hxz_zp=(cpml_b_mz_zp).*Psi_hxz_zp+(cpml_a_mz_zp).*(Ey(:,:,cpmlzpvec+2)-Ey(:,:,cpmlzpvec+1));
Hx(:,:,cpmlzpvec+1)=Hx(:,:,cpmlzpvec+1)+CPsi_hxz_zp.* Psi_hxz_zp;
Hy(:,:,cpmlzpvec+1)=Hy(:,:,cpmlzpvec+1)+CPsi_hyz_zp.* Psi_hyz_zp;
% 
% for i = 1: n_cpml_xn
...
Psi_eyx_xp=cpml_b_ex_xp.*Psi_eyx_xp+cpml_a_ex_xp.*(Hz(cpmlxpvec+1,:,:) - Hz(cpmlxpvec,:,:));
Psi_ezx_xp=cpml_b_ex_xp.*Psi_ezx_xp+cpml_a_ex_xp.*(Hy(cpmlxpvec+1,:,:) - Hy(cpmlxpvec,:,:));
Ey(cpmlxpvec+1,:,:)=Ey(cpmlxpvec+1,:,:)+CPsi_eyx_xp.*Psi_eyx_xp;
Ez(cpmlxpvec+1,:,:)=Ez(cpmlxpvec+1,:,:)+CPsi_ezx_xp.*Psi_ezx_xp;

Psi_exy_yn(:,:,:) =cpml_b_ey_yn.*Psi_exy_yn+cpml_a_ey_yn.*(Hz(:,cpmlynvec+1,:) - Hz(:,cpmlynvec,:));
Psi_ezy_yn(:,:,:) =cpml_b_ey_yn.*Psi_ezy_yn+cpml_a_ey_yn.*(Hx(:,cpmlynvec+1,:) - Hx(:,cpmlynvec,:));
Ex (:,cpmlynvec+1,:)=Ex(:,cpmlynvec+1,:)+ CPsi_exy_yn .* Psi_exy_yn;
Ez (:,cpmlynvec+1,:)=Ez(:,cpmlynvec+1,:)+ CPsi_ezy_yn .* Psi_ezy_yn; 

Psi_exy_yp=cpml_b_ey_yp.*Psi_exy_yp+cpml_a_ey_yp.*(Hz(:,1+cpmlypvec,:) - Hz(:,cpmlypvec,:));
Psi_ezy_yp=cpml_b_ey_yp.*Psi_ezy_yp+cpml_a_ey_yp.*(Hx(:,1+cpmlypvec,:) - Hx(:,cpmlypvec,:));
Ex(:,cpmlypvec+1,:)=Ex(:,cpmlypvec+1,:)+CPsi_exy_yp.*Psi_exy_yp;
Ez(:,cpmlypvec+1,:)=Ez(:,cpmlypvec+1,:)+CPsi_ezy_yp.*Psi_ezy_yp;

Psi_exz_zn =cpml_b_ez_zn.*Psi_exz_zn+cpml_a_ez_zn.*(Hy(:,:,cpmlznvec+1) - Hy(:,:,cpmlznvec));
Psi_eyz_zn =cpml_b_ez_zn.*Psi_eyz_zn+cpml_a_ez_zn.*(Hx(:,:,cpmlznvec+1) - Hx(:,:,cpmlznvec));
Ex (:,:,cpmlznvec+1) = Ex(:,:,cpmlznvec+1)+ CPsi_exz_zn .* Psi_exz_zn;
Ey (:,:,cpmlznvec+1)= Ey(:,:,cpmlznvec+1)+ CPsi_eyz_zn .* Psi_eyz_zn; 

Psi_exz_zp=cpml_b_ez_zp.*Psi_exz_zp+cpml_a_ez_zp.*(Hy(:,:,1+cpmlzpvec) - Hy(:,:,cpmlzpvec));
Psi_eyz_zp=cpml_b_ez_zp.*Psi_eyz_zp+cpml_a_ez_zp.*(Hx(:,:,1+cpmlzpvec) - Hx(:,:,cpmlzpvec));
Ex(:,:,cpmlzpvec+1)=Ex(:,:,cpmlzpvec+1)+CPsi_exz_zp.*Psi_exz_zp;
Ey(:,:,cpmlzpvec+1)=Ey(:,:,cpmlzpvec+1)+CPsi_eyz_zp.*Psi_eyz_zp;
...
Ezp=Ez;
%% update source
Hx((nx+1)./2,((ny+1)./2),n_cpml_zn+2)=Chxh((nx+1)./2,((ny+1)./2),n_cpml_zn+2).*Hx((nx+1)./2,((ny+1)./2),n_cpml_zn+2)+Chxey((nx+1)./2,((ny+1)./2),n_cpml_zn+2) .*(Ey((nx+1)./2,((ny+1)./2)+1,n_cpml_zn+3)- voltage.*sin(number*dt*omega)./dy)+...
    Chxez((nx+1)./2,((ny+1)./2),n_cpml_zn+2).*(Ez((nx+1)./2,((ny+1)./2)+1,n_cpml_zn+2)- Ez ((nx+1)./2,((ny+1)./2),n_cpml_zn+2));
Hy(((nx+1)./2),((ny+1)./2),n_cpml_zn+2)=Chyh(((nx+1)./2),((ny+1)./2),n_cpml_zn+2).*Hy(((nx+1)./2),((ny+1)./2),n_cpml_zn+2)+Chyez(((nx+1)./2),((ny+1)./2),n_cpml_zn+2) .*(Ez (((nx+1)./2)+1,((ny+1)./2),n_cpml_zn+2)- Ez (((nx+1)./2),((ny+1)./2),n_cpml_zn+2))+...
    Chyex(((nx+1)./2),((ny+1)./2),n_cpml_zn+2).*(Ex(((nx+1)./2),((ny+1)./2),n_cpml_zn+3)- voltage.*sin(number*dt*omega)./dx);
Hx((nx+1)./2,((ny+1)./2)-1,n_cpml_zn+2)=Chxh((nx+1)./2,((ny+1)./2)+1,n_cpml_zn+2).*Hx((nx+1)./2,((ny+1)./2)+1,n_cpml_zn+2)+Chxey((nx+1)./2,((ny+1)./2)-1,n_cpml_zn+2) .*(Ey ((nx+1)./2,((ny+1)./2)-1,n_cpml_zn+3)- voltage.*sin(number*dt*omega)./dy)+...
    Chxez((nx+1)./2,((ny+1)./2)-1,n_cpml_zn+2).*(Ez((nx+1)./2,((ny+1)./2),n_cpml_zn+2)- Ez ((nx+1)./2,((ny+1)./2)-1,n_cpml_zn+2));
Hy(((nx+1)./2)-1,((ny+1)./2),n_cpml_zn+2)=Chyh(((nx+1)./2)-1,((ny+1)./2),n_cpml_zn+2).*Hy(((nx+1)./2)-1,((ny+1)./2),n_cpml_zn+2)+Chyez(((nx+1)./2),((ny+1)./2)+1,n_cpml_zn+2).*(Ez(((nx+1)./2),((ny+1)./2),n_cpml_zn+2)- Ez(((nx+1)./2)-1,((ny+1)./2),n_cpml_zn+2))+...
    Chyex(((nx+1)./2)-1,((ny+1)./2),n_cpml_zn+2).*(Ex(((nx+1)./2)-1,((ny+1)./2),n_cpml_zn+3)- voltage.*sin(number*dt*omega)./dx);

Ex(nxp1/2,nyp1/2,n_cpml_zn+2)=Ex(nxp1/2,nyp1/2,n_cpml_zn+2)+voltage *sin((number * dt - t0) * omega)/dz; % Sine source

%% capture_and_display_sampled_fields
     if mod(number,10000)==0||number==totalTimeStep
       toc
Hx_sample=Hx;
Hy_sample=Hy;
Hz_sample=Hz;
...
title({'Ex'}); 
drawnow
% savefig(Hh_1,'three_layer_dispersive_skin_fdtd_cpml_sinesoidal_3D_3D.fig')
...
grid on
% savefig(Hh_2,'three_layer_dispersive_skin_fdtd_cpml_sinesoidal_3D_2D.fig')
   
for i=1:nz
Ez_sample_2D(i)=Ez(nxp1/2,nyp1/2,i) ;
end
% Hh_3(number/10)=figure
hold on
plot(zcoor(1:nz),Ez_sample_2D)
